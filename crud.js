...
class Inventario {
      ...
      document.getElementById("t-render").innerHTML += `
        <tr>
            ...
            <td class="text-end">
                ...
                <!-- Eliminar -->
                <button type="button" class="btn btn-tertiary color-danger p-2" onclick="Inventario.Eliminar(${nombre})">
  ...
  static edit(name) {
    ...
    Inventario.forEach((i) => {
      if (i.nombre == name) {
        document.getElementById("ID DEL INPUT A LLENAR CON DATA").value =
          i.nombre;
        document.getElementById("ID DEL INPUT A LLENAR CON DATA").value =
          i.marca;
        document.getElementById("ID DEL INPUT A LLENAR CON DATA").value =
